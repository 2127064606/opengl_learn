...
void MyModel::loadModel(const char* path){
    ...
    if(!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode){
        ERROR(std::string("Assimp: Failed to load model: ") + importer.GetErrorString());
        return;
    }
...
std::vector<Texture> MyModel::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string _typename){
    std::vector<Texture> textures;
    for(unsigned int i = 0; i < mat->GetTextureCount(type); i++){
        ...
        for(unsigned int j = 0; j < loadedTextures.size(); j++){
            if(strcmp(loadedTextures[j].path.c_str(), str.C_Str()) == 0){
        ...
        if(!skip){
        Texture texture;
        INFO(getFullPath(directory, std::string(str.C_Str())).c_str());
